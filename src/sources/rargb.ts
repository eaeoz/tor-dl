...
export class RARGBScraper implements SourceScraper {
  name = 'RARGB';

  async search(query: string, category?: string): Promise<TorrentResult[]> {
    try {
      const catMap: Record<string, string> = { movie: 'movies', tv: 'tv', games: 'games', music: 'music', anime: 'anime', apps: 'apps', documentary: 'documentaries', other: 'other', xxx: 'xxx' };
      const catParam = category && catMap[category] ? `&category[]=${catMap[category]}` : '';
      const url = `https://rargb.to/search/?search=${encodeURIComponent(query)}${catParam}`;
      ...
      $('table.lista2t tr.lista2').each((i, el) => {
        const titleEl = $(el).find('td:nth-child(2) a');
        const title = titleEl.text().trim();
        const size = $(el).find('td:nth-child(5)').text().trim();
        const seedsText = $(el).find('td:nth-child(6)').text().trim();
        const seeds = parseInt(seedsText) || 0;
        const peers = parseInt($(el).find('td:nth-child(7)').text().trim()) || 0;
        const link = 'https://rargb.to' + (titleEl.attr('href') || '');
        
        if (title && !title.includes('imdb') && !title.includes('tvcache') && !title.includes('clickstream')) {
          results.push({
      ...
      console.error(`RARGB search error: ${message}`);
      return [];
    }
  }
...
export default new RARGBScraper();